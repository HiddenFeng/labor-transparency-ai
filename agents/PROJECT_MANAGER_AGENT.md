...
4. `docs/v0_8/CHANGE-v0.8.1-tencent-free-deployment.md`, `docs/v0_8/CHANGE-v0.8.1-user-company-auto-research.md`, `docs/v0_8/CHANGE-v0.8.2-autonomous-company-intelligence.md`, and the current v0.8 deployment/privacy/company-research docs
...
Treat `docs/v0_8/source-registry.json`, `qa/v0_8/company-intelligence-audit.json`, current live-source evidence under `qa/v0_8/`, `/api/research/coverage`, `/api/research/status`, and the persisted per-company research lifecycle as the current sources of truth. A newly created real company should enter `QUEUED`, return `researchDispatch=QUEUE_SENT`, and progress through the Cloudflare Queue consumer without a manual enqueue/run/review step; scheduled triggers only re-enqueue missed, stale, failed or refresh-eligible work. Production may automatically publish only `MACHINE_VERIFIED_REFERENCE` fields under the current fail-closed identity policy and `SOURCE_SIGNAL` statements whose wording preserves the upstream record's procedural scope. Ambiguous identity, country conflicts, legal conclusions and unsupported relationships must remain candidates/unknown. Raw records and private material stay restricted. Optional Python/human review is an audit tool, never a production availability dependency.
