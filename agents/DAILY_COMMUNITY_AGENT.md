...
2. Prioritize existing **China company spaces** and run the approved China investigation collectors first:
   ...
   - Shenzhen Stock Exchange listing/disclosure reference: `python3 scripts/community_agent/cn_szse_listing_daily.py --publish --output history/daily/YYYY-MM-DD/cn-szse-listing.json`
   ...
   These China collectors must keep `OFFICIAL_SOURCE_REFERENCE`, `OFFICIAL_SOURCE_RELATION`, and `OFFICIAL_SOURCE_EVENT` separate. SSE candidate security codes are discovery only and require an exact official company-overview full-name match; SZSE references require one exact full-company-name match in the official stock-list report and duplicate exact rows fail closed. A recall/penalty event supports only its specific official record; zero matches never means “not listed / no recalls / no penalties / no problems exist”.
