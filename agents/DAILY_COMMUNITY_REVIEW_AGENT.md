...
5. For China companies, verify that SSE/SZSE listing-reference, NMPA UDI, SAMR recall and CSRC penalty collectors were attempted and that `chinaInvestigation` gaps remain explicit. Exchange references must stay listing/disclosure scoped and must not upgrade legal identity. GSXT/CNIPA/government-procurement CAPTCHA or anti-bot paths remain verification gaps rather than automation failures.
6. If the 18:00 work was incomplete, continue the same bounded work directly. Re-running the SSE/SZSE listing-reference, NMPA UDI, SAMR recall, CSRC penalty, EU TED, and Japan NTA collectors is safe and idempotent because public official records use deterministic identities. The approved daily CLI may use the bounded Wrangler-D1 fallback when normal production API networking from the Mac is unavailable. Never improvise raw D1 mutations; if both approved paths fail, record the failure and keep the day `PARTIAL`.
