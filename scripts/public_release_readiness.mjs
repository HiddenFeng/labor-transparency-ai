...
export async function publicReleaseReadiness(){
  ...
  return {status:failures.length?'NOT_READY':'PUBLIC_GOVERNANCE_READY_SOURCE_CANDIDATE',version:`v${version}`,acceptedProduction:acceptedVersion?`v${acceptedVersion}`:'UNKNOWN',phaseOCandidateProductionAccepted:acceptedVersion===version,governanceFiles:REQUIRED_PUBLIC_GOVERNANCE_FILES.length,githubGovernanceFiles:REQUIRED_GITHUB_GOVERNANCE_FILES.length,publicSitePolicyMirrors:PUBLIC_SITE_POLICY_MIRRORS.length,missing,tooSmall,forbiddenTracked,failures,dockerContainerE2E:'BLOCKED_BY_EXTERNAL_DOCKER_HUB_PULL_PATH',nextGate:acceptedVersion===version?'CLEAN_SOURCE_RELEASE_CANDIDATE_OR_SEPARATE_REFERENCE_PRODUCTION_RELEASE_GATE':'VERSIONED_SOURCE_CANDIDATE_PUBLICATION_AND_CI_BEFORE_REFERENCE_PRODUCTION_RELEASE_GATE'};
}
