...
export async function publicReleaseReadiness(){
  ...
  const pkg=JSON.parse(await read('sites-app/package.json'));const version=String(pkg.version||'');
  ...
  for(const [label,text] of [['README',readme],['PROJECT_CONTINUITY',continuity],['RELEASE_POLICY',release]])if(!text.includes(`v${version}`))failures.push(`${label} does not name current v${version} baseline`);
  ...
  return {status:failures.length?'NOT_READY':'PUBLIC_GOVERNANCE_READY_SOURCE_CANDIDATE',version:`v${version}`,acceptedProduction:`v${version}`,phaseOCandidateProductionAccepted:false,governanceFiles:REQUIRED_PUBLIC_GOVERNANCE_FILES.length,githubGovernanceFiles:REQUIRED_GITHUB_GOVERNANCE_FILES.length,publicSitePolicyMirrors:PUBLIC_SITE_POLICY_MIRRORS.length,missing,tooSmall,forbiddenTracked,failures,dockerContainerE2E:'BLOCKED_BY_EXTERNAL_DOCKER_HUB_PULL_PATH',nextGate:'CLEAN_SOURCE_RELEASE_CANDIDATE_OR_SEPARATE_REFERENCE_PRODUCTION_RELEASE_GATE'};
}
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){const result=await publicReleaseReadiness();console.log(JSON.stringify(result,null,2));if(result.status!=='PUBLIC_GOVERNANCE_READY_SOURCE_CANDIDATE')process.exitCode=1;}
