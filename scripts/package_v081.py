# -*- coding: utf-8 -*-
"""Build a source-only v0.8.1 independent deployment + multi-source research RC."""
from __future__ import annotations
import argparse, hashlib, json, zipfile
from pathlib import Path
from release_files import release_files
ROOT=Path(__file__).resolve().parents[1]
PREFIX=Path('labor-transparency-v0.8.1-independent-research-rc1')

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out',default='/tmp/labor-transparency-v0.8.1-independent-research-rc1.zip');args=ap.parse_args()
    out=Path(args.out).expanduser().resolve();out.parent.mkdir(parents=True,exist_ok=True)
    if out.exists():out.unlink()
    files=release_files(ROOT)
    manifest={'version':'0.8.1-rc.1','purpose':'independent static frontend + Cloudflare Worker/D1 + bounded multi-source company research candidate','status':'NOT_PUBLICLY_DEPLOYED','files':{}}
    for src in files:
