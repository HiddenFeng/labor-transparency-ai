# -*- coding: utf-8 -*-
"""Build a self-contained ChatGPT Sites v0.7.2 handoff zip from explicit allowlists."""
...
EXTRA=[
    ...
    ROOT/'docs'/'v0_7'/'CHANGE-v0.7.2-public-ui.md',
    ROOT/'docs'/'v0_7'/'CHANGE-v0.7.1-anonymous-advisory.md',
    ROOT/'docs'/'v0_7'/'SITES交互版设计与边界.md',ROOT/'docs'/'v0_7'/'交付与验收.md',
    ROOT/'qa'/'v0_7'/'verification.json',ROOT/'qa'/'v0_7'/'sites-browser-v072.json',
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out',default='/tmp/labor-transparency-v0.7.2-sites-rc1.zip');args=ap.parse_args()
    out=Path(args.out).expanduser().resolve();out.parent.mkdir(parents=True,exist_ok=True)
    if out.exists():out.unlink()
    files=collect();manifest={'version':'0.7.2-rc.1','purpose':'ChatGPT Sites worker-facing public-interest handoff candidate','privacy':'no real sensitive private information or attachments','files':{}}
    for src,rel in files:manifest['files'][str(rel)]=hashlib.sha256(src.read_bytes()).hexdigest()
    prefix=Path('labor-transparency-v0.7.2-sites-rc1')
