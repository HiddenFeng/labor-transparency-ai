...
const cfg=await req('/api/config');assert.equal(cfg.status,200);assert.equal(cfg.data.version,'0.8.2-rc.1');csrf=cfg.data.csrfToken;
...
assert.equal(record.companyId,companyId);assert.ok(['AUTO_READY','AUTO_READY_WITH_SOURCE_GAPS'].includes(record.status),record.status);assert.ok(record.candidateCount>0);assert.equal(record.reviewRequired,false);assert.ok(record.intelligence);assert.ok(record.intelligence.coverage.machineVerifiedFacts>0);assert.ok(record.intelligence.coverage.sourceSignals>0);
...
const publicStatus=await req('/api/research/status');assert.equal(publicStatus.status,200);assert.equal(publicStatus.data.companiesTracked,1);assert.equal(publicStatus.data.pendingReview,0);assert.equal(publicStatus.data.autonomousReady,1);assert.ok(publicStatus.data.machineVerifiedFacts>0);assert.ok(publicStatus.data.sourceSignals>0);
...
const result={status:'PASS_LIVE_AUTONOMOUS_COMPANY_INTELLIGENCE',version:cfg.data.version,company:{id:companyId,name:'Starbucks Corporation',region:'US'},trigger:'USER_CREATE_DIRECT_QUEUE_CONSUMER',researchDispatch:created.data.researchDispatch,researchStatus:record.status,processedCompanies:1,candidateCount:record.candidateCount,sourceErrors:record.sourceErrorCount,machineVerifiedFacts:record.intelligence.coverage.machineVerifiedFacts,sourceSignals:record.intelligence.coverage.sourceSignals,identityStatus:record.intelligence.identity.status,clusterCount:record.intelligence.clusters?.length||0,healthStatus:autonomyHealth.data.status,unattendedOperation:autonomyHealth.data.unattendedOperation,providers,publicProjectionSafe:true,publicProviderCount:publicCompany.research.providers.length,reviewRequired:false,boundary:'Live Worker Queue consumer automatically derives only narrow machine-verifiable reference facts and bounded source signals. Ambiguous or legally interpretive material remains candidate/unknown without a named reviewer gate.'};
