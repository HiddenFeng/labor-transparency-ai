...
ARGS=(deploy -c "$CONFIG" --strict --secrets-file "$SECRETS" --var "LTP_ENV:production" --var "LTP_ALLOWED_ORIGINS:$LTP_FRONTEND_ORIGINS" --var "LTP_COOKIE_SECURE:true" --var "LTP_SEED:false" --var "LTP_RESEARCH_SCHEDULED:true" --var "LTP_RESEARCH_MAX_COMPANIES_PER_RUN:${LTP_RESEARCH_MAX_COMPANIES_PER_RUN:-2}")
