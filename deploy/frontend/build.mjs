...
await fs.writeFile(path.join(out,'runtime-config.js'),`globalThis.__LTP_CONFIG__=${JSON.stringify({apiBase,deployment:mode})};\n`);
...
console.log(JSON.stringify({status:'BUILT',out,apiBase:apiBase||'(same-origin)',proxyApiOrigin:proxyApiOrigin||'',files:(await fs.readdir(out)).length}));
