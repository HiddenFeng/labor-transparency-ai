...
**无 ICP / 无正式中国大陆域名与服务商资格时，不能诚实承诺“中国大陆稳定可访问”。** 本轮已经把这一点从纸面判断推进到真实网络测试：当前大陆网络可访问 EdgeOne 网络，但直接访问 `vercel.app` / `workers.dev` 失败或超时；账号迁移前的阶段性 GitHub Pages 镜像已经退出当前公开入口。
...
- EdgeOne 正式账号/项目 claim 已完成；下一步只使用项目拥有的自定义域名，不使用带个人历史含义的现有域名来省略治理步骤。
- 取得 ICP 后把前端部署/绑定到 EdgeOne 中国大陆可用区或全球含中国大陆区域。
- EdgeOne 官方要求中国大陆/全球含大陆的自定义域名完成 ICP。
- 本轮 PoC 已证明 EdgeOne Edge Function 可以把同源 `/api/*` 转发到 Cloudflare Worker；因此它不仅能改善静态前端，也能作为跨境动态 API relay。但长期稳定性、合规和 SLA 仍必须在正式项目/域名上实测。
