...
function appendOfficialReferences(root,rows){
  ...
  for(const item of rows){
    ...
    const meta=[f.exchange&&`交易所 ${f.exchange}`,f.securityCode&&`证券代码 ${f.securityCode}`,f.securityAbbreviation&&`证券简称 ${f.securityAbbreviation}`,f.board&&`板块 ${f.board}`,f.listingDate&&`上市日期 ${f.listingDate}`,f.listingStatus&&`上市状态 ${f.listingStatus}`,f.industry&&`行业 ${f.industry}`,f.registeredArea&&`注册地区 ${f.registeredArea}`,f.province&&`省份 ${f.province}`,f.city&&`城市 ${f.city}`,f.registeredAddress&&`注册地址 ${f.registeredAddress}`,f.corporateNumber&&`法人番号 ${f.corporateNumber}`,f.prefectureName&&`都道府县 ${f.prefectureName}`,f.cityName&&`市区町村 ${f.cityName}`,f.assignmentDate&&`指定日 ${f.assignmentDate}`,f.closeDate&&`注销日 ${f.closeDate}`,f.enName&&`英文名 ${f.enName}`,f.englishName&&`英文名 ${f.englishName}`].filter(Boolean);if(meta.length)card.append(text('p',meta.join(' · ')));
