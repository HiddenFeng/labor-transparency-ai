...
export const COMPANY_RESEARCH_COVERAGE = Object.freeze({
  ...
  sections:[
    ...
    {key:'official_references',label:'官方登记 / 上市披露参考',status:'AUTOMATED_PARTIAL_REGION_LIMITED',source:'中国上海证券交易所公司概况 + 深圳证券交易所股票列表 + 日本国税庁法人番号日次差分；GSXT 保留官方核验入口',scope:'上交所必须由证券代码候选对应的官方公司概况完整名称精确复核；深交所只接受官方股票列表中唯一完整公司名称精确匹配；日本法人番号日差分只形成当日精确名称参考。这些参考都不自动升级全国法律主体身份，也不代表企业整体质量、合规或投资价值。'},
  ...
  claimBoundary:'生产系统无人值守运行：严格机器规则满足时可自动发布窄范围法律实体/申报参考事实；每日本地Agent可把上交所/深交所上市披露与日本法人番号日差分的严格匹配写入OFFICIAL_SOURCE_REFERENCE层，把NMPA UDI与EU TED的精确官方关系写入OFFICIAL_SOURCE_RELATION层，并把SAMR召回/CSRC行政处罚等精确绑定的具体官方记录写入OFFICIAL_SOURCE_EVENT层。任何单条参考、关系或事件都不能自动扩张为企业整体违法、风险、劳动质量、产品质量或投资价值结论；没有来源、地区不适用、主体歧义和来源不可达都会显式保留为未知。'
});
