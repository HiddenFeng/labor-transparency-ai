...
const manifest={version:'0.8.2-rc.1',generatedAt:new Date().toISOString(),files:{}};for(const p of files.sort()){const rel=path.relative(out,p);manifest.files[rel]=crypto.createHash('sha256').update(await fs.readFile(p)).digest('hex')}
