...
test('real HTTP contribution -> review -> redistribution -> correction -> persistence loop',async t=>{
  ...
  assert.equal(cfg.version,'0.8.8-rc.1');assert.equal(cfg.capabilities.attachments,false);assert.equal(cfg.capabilities.privateSensitiveInfo,false);assert.equal(cfg.capabilities.anonymousAdvisory,true);assert.equal(cfg.capabilities.workerPerspectiveSignals,true);assert.equal(cfg.capabilities.productMarket,true);assert.equal(cfg.capabilities.federatedPublicEvidence,true);assert.equal(cfg.capabilities.communityFeedback,true);assert.equal(cfg.capabilities.officialReferences,true);assert.equal(cfg.capabilities.officialRelations,true);assert.equal(cfg.capabilities.officialEvents,true);assert.equal(cfg.capabilities.publicAnnouncements,true);
  const federationEmpty=await client.request('/api/federation/evidence');assert.equal(federationEmpty.status,200);assert.deepEqual(federationEmpty.data.items,[]);assert.deepEqual(federationEmpty.data.sources,[]);assert.match(federationEmpty.data.boundary,/不包含用户身份/);
  ...
  const workerBallot=await client.request(`/api/companies/${cid}/ballot`,{method:'POST',body:{direction:'negative',signalType:'worker'}});assert.equal(workerBallot.status,200);assert.equal(workerBallot.data.signalType,'worker');
  ...
  const angerProducts=await client.request('/api/product-market?lane=worker_negative');assert.equal(angerProducts.status,200);assert.equal(angerProducts.data.items.length,1);assert.equal(angerProducts.data.items[0].workerPerspective.negative,1);assert.equal(angerProducts.data.items[0].community.positive,1);assert.match(angerProducts.data.method,/情绪\/感受/);assert.equal(JSON.stringify(angerProducts.data).includes('owner'),false);
  ...
  const evidenceProducts=await client.request('/api/product-market?lane=evidence');assert.equal(evidenceProducts.status,200);assert.equal(evidenceProducts.data.items.length,1);assert.equal(evidenceProducts.data.items[0].labourEvidence.strongestEvidence,'E3');assert.equal(evidenceProducts.data.items[0].labourEvidence.verifiedPositiveClaims.length,1);
