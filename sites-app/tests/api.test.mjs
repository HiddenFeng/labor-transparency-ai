...
test('real HTTP contribution -> review -> redistribution -> correction -> persistence loop',async t=>{
  ...
  assert.equal(cfg.version,'0.8.4-rc.2');assert.equal(cfg.capabilities.attachments,false);assert.equal(cfg.capabilities.privateSensitiveInfo,false);assert.equal(cfg.capabilities.anonymousAdvisory,true);assert.equal(cfg.capabilities.communityFeedback,true);assert.equal(cfg.capabilities.officialRelations,true);assert.equal(cfg.capabilities.publicAnnouncements,true);
