...
assert.equal(cfg.status,200);assert.equal(cfg.data.version,'0.9.0-rc.1');assert.equal(cfg.data.mode,'CLOUDFLARE_WORKER_D1');assert.equal(cfg.data.capabilities.unattendedCompanyIntelligence,true);assert.equal(cfg.data.capabilities.workerPerspectiveSignals,true);assert.equal(cfg.data.capabilities.productMarket,true);assert.equal(cfg.data.capabilities.federatedPublicEvidence,true);assert.equal(cfg.data.capabilities.communityFeedback,true);assert.equal(cfg.data.capabilities.officialReferences,true);assert.equal(cfg.data.capabilities.officialRelations,true);assert.equal(cfg.data.capabilities.officialEvents,true);assert.equal(cfg.data.capabilities.publicAnnouncements,true);assert.ok(cookie.startsWith('ltp_session='));assert.equal(cfg.headers.get('access-control-allow-origin'),origin);csrf=cfg.data.csrfToken;assert.match(csrf,/^[a-f0-9]{64}$/);
const federationEmpty=await req('/api/federation/evidence');assert.equal(federationEmpty.status,200);assert.deepEqual(federationEmpty.data.items,[]);assert.deepEqual(federationEmpty.data.sources,[]);assert.match(federationEmpty.data.boundary,/不包含用户身份/);
...
const workerBallot=await req(`/api/companies/${companyId}/ballot`,{method:'POST',body:{direction:'positive',signalType:'worker'}});assert.equal(workerBallot.status,200);assert.equal(workerBallot.data.signalType,'worker');
const product=await req('/api/contributions',{method:'POST',body:{companyId,kind:'product',title:'本地 smoke 产品',description:'用于验证产品页与劳动者视角 D1 持久化。',scope:'',periodStart:'',periodEnd:'',direction:'neutral',dimension:'other',productId:'',brandId:'',relationType:'',relation:'',category:'测试产品',sources:[],public:true,consent:true,shareConsent:false,rights:'own_summary',rightsNote:'',creditName:'自动化测试'}});assert.equal(product.status,200);
const productMarket=await req('/api/product-market?lane=worker_positive');assert.equal(productMarket.status,200);const smokeProduct=productMarket.data.items.find(x=>x.company.id===companyId&&x.product.id===product.data.item.id);assert.ok(smokeProduct);assert.equal(smokeProduct.workerPerspective.positive,1);assert.equal(smokeProduct.community.negative,1);assert.match(productMarket.data.method,/情绪\/感受/);
...
const companyDetail=await req(`/api/companies/${companyId}`);assert.equal(companyDetail.status,200);assert.equal(companyDetail.data.company.id,companyId);assert.equal(companyDetail.data.community.negative,1);assert.equal(companyDetail.data.community.participants,1);assert.equal(companyDetail.data.workerPerspective.positive,1);assert.equal(companyDetail.data.workerPerspective.participants,1);assert.equal(companyDetail.data.research.status,'QUEUED');assert.ok(companyDetail.data.contributions.products.some(x=>x.id===product.data.item.id));assert.ok(companyDetail.data.contributions.labourClaims.some(x=>x.id===cid));const detailJson=JSON.stringify(companyDetail.data);assert.equal(detailJson.includes('owner'),false);assert.equal(detailJson.includes('rightsNote'),false);assert.match(companyDetail.data.boundary,/社区反馈/);
