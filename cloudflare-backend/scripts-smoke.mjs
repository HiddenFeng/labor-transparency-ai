...
assert.equal(cfg.status,200);assert.equal(cfg.data.version,'0.8.8-rc.1');assert.equal(cfg.data.mode,'CLOUDFLARE_WORKER_D1');assert.equal(cfg.data.capabilities.unattendedCompanyIntelligence,true);assert.equal(cfg.data.capabilities.communityFeedback,true);assert.equal(cfg.data.capabilities.officialReferences,true);assert.equal(cfg.data.capabilities.officialRelations,true);assert.equal(cfg.data.capabilities.officialEvents,true);assert.equal(cfg.data.capabilities.publicAnnouncements,true);assert.ok(cookie.startsWith('ltp_session='));assert.equal(cfg.headers.get('access-control-allow-origin'),origin);csrf=cfg.data.csrfToken;assert.match(csrf,/^[a-f0-9]{64}$/);
