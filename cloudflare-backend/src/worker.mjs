...
const BACKEND_VERSION='0.8.5-rc.1';
...
function communityAgentState(state){
  ...
  return {
    ...
    officialReferenceCount:Number(state.officialReferences?.length||0),
    officialRelationCount:Number(state.officialRelations?.length||0),
    officialEventCount:Number(state.officialEvents?.length||0),
...
async function handleApi(request,env){
  ...
  if(request.method==='GET'&&url.pathname==='/api/config') response=json(request,env,200,{version:BACKEND_VERSION,domainVersion:DOMAIN_VERSION,mode:'CLOUDFLARE_WORKER_D1',csrfToken:ctx.csrf,cookieSecure:String(env.LTP_COOKIE_SECURE||'true')!=='false',capabilities:{companies:true,ballots:true,contributions:true,brandContributions:true,officialReferences:true,officialRelations:true,officialEvents:true,communityFeedback:true,publicAnnouncements:true,review:true,publicData:true,anonymousAdvisory:true,advisoryDailyReports:true,scheduledAdvisory:true,automaticCompanyResearch:researchQueueEnabled(env),unattendedCompanyIntelligence:true,companyResearchQueue:researchQueueEnabled(env),scheduledCompanyResearch:String(env.LTP_RESEARCH_SCHEDULED||'false')==='true',attachments:false,privateSensitiveInfo:false},privacy:'匿名辅导与社区意见仅接收非敏感结构化内容；不接收真实姓名、私人联系方式、身份证明、健康/支付信息或敏感附件'});
    ...
    const companyId=decodeURIComponent(url.pathname.split('/').at(-1));const detail=publicCompanyDetail(await store.read(),companyId);
    response=detail?json(request,env,200,detail):json(request,env,404,{error:'公司空间不存在'});
  }
    ...
    else if(request.method==='POST'&&url.pathname==='/api/community-agent/official-references'){
      if(!env.LTP_COMMUNITY_AGENT_TOKEN||!safeEqual(authToken(request),env.LTP_COMMUNITY_AGENT_TOKEN))response=json(request,env,403,{error:'需要社区运营 Agent 凭据'});
      else {const input=await bodyJson(request);const out=await store.transaction(s=>upsertOfficialReferences(s,input));response=json(request,env,200,{savedCount:out.savedCount});}
    }
    ...
    else if(request.method==='POST'&&url.pathname==='/api/community-agent/official-events'){
      if(!env.LTP_COMMUNITY_AGENT_TOKEN||!safeEqual(authToken(request),env.LTP_COMMUNITY_AGENT_TOKEN))response=json(request,env,403,{error:'需要社区运营 Agent 凭据'});
      else {const input=await bodyJson(request);const out=await store.transaction(s=>upsertOfficialEvents(s,input));response=json(request,env,200,{savedCount:out.savedCount});}
    }
